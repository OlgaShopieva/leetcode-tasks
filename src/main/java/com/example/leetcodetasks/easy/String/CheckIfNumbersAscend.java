 ...
 * Every token is either a positive number consisting of digits 0-9 with no leading zeros, or a word consisting
 ...
 */
public class CheckIfNumbersAscend {
    ...
    public boolean areNumbersAscending(String s) {
        ...
        for(String elem : str) {
            if(Character.isDigit(elem.charAt(0))) {
                int right = Integer.parseInt(elem);
                if(!stack.empty()) {
                    int left = stack.peek();
                    if(right <= left) {
                        return false;
