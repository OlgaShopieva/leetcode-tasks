 ...
 * Your task is to convert the string into an integer by a special process, and then transform it
 * by summing its digits repeatedly k times. More specifically, perform the following steps:
...
public class SumOfDigitsAfterConvert {
    ...
    public static int getLucky(String s, int k) {
        ...
        for(char ch: s.toCharArray()) {
            sb.append(positionMap[ch]);
        }
        char[] chars = new char[sb.length()];
        sb.getChars(0, sb.length(), chars, 0);

        int i = 1;
        int result = getSum(chars);
        while (i < k) {
            String numberStr = Integer.toString(result);
            char[] charArray = numberStr.toCharArray();
            result = getSum(charArray);
            i++;
        }
